...
собственные параметры, checkpoint, CSV и qualitative export, поэтому
эксперименты не перезаписывают друг друга.

Основной сценарий находится в `notebooks/run_all_colab.ipynb`. Разделы 1–9
подготавливают GPU, Google Drive, код из ветки `codex/deterministic-training-plan`, Cityscapes,
split и проверяют датасет, а раздел 10 загружает общие функции запуска. После этого
...
Конфигурация, история обучения, сведения об окружении и таблицы оценок остаются
непосредственно в каталоге запуска и не дублируются в отдельное хранилище.
...
Новое имя запуска создаёт новый эксперимент и детерминированный
`training_plan.csv` для всех его эпох. То же имя с `--resume` продолжает
его из `last.pt`, используя порядок нужной абсолютной эпохи.
...
эпох и общий target epochs. План до эпохи выбранного checkpoint копируется,
а новые эпохи детерминированно добавляются.

`training_plan.csv` в каждой эпохе содержит все train-`image_id` ровно по одному разу.
Порядок каждой эпохи уникален и строится из пары `seed`/номер эпохи. План сверяется
напрямую с `image_id` из `split_manifest.csv`; хеши и fingerprint не используются.
При одинаковых split, `seed` и числе эпох модели получают одинаковый порядок изображений.
Для одинаковых robust-настроек также совпадают выбранные изображения, виды и параметры
искажений. Старые run-ы без плана остаются доступны в предыдущей ветке, но в этой ветке
не возобновляются и не используются как источник continue. Их по-прежнему можно оценивать
и экспортировать, потому что эти операции не используют план обучения.
...
## Baseline- и robust-обучение

Baseline-run не использует аугментации: train-transform состоит только из resize,
ImageNet normalize и tensor conversion. Блока `augmentation` в baseline-`run_config.yaml` нет.
...
В настройках модели ноутбука для robust-run поставьте:
...
При `corrupted_fraction: 0.5` половина train-изображений каждой эпохи остаётся clean,
а оставшаяся половина заранее и максимально равномерно распределяется между пятью seen-искажениями:
`darkness`, `brightness`, `gaussian_blur`, `gaussian_noise`, `jpeg_compression`.
Точные виды, параметры и `noise_seed` хранятся в плане. `fog` остаётся unseen-искажением
